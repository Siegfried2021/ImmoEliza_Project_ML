# create a heatmap of houses with swimming pool in Belgium

import geopandas
import pandas as pd

geo_df=geopandas.read_file("/home/siegfried2021/Bureau/BeCode_AI/Projets/ImmoEliza/Preprocessing-Visualization/data/BELGIUM_-_Municipalities.geojson")
...
/home/siegfried2021/Bureau/BeCode_AI/Projets/ImmoEliza/ImmoElizaML/data/REFNIS_2019.xls
...
df_pool=df_ref.groupby(by=["Refnis code","Nom commune"]).SwimmingPool.mean().reset_index().drop_duplicates(subset="Refnis code",keep="first")
df_pool["SwimmingPool"]=df_pool.SwimmingPool.apply(lambda x:round(x,2))
df_pool["Refnis code"]=df_pool["Refnis code"].astype(int)

geo_price = geo_base.merge(df_pool,on="Refnis code",how="left")

geo_price.plot(column="SwimmingPool",legend=True,missing_kwds={"color": "lightgrey", "edgecolor": "red", "hatch": "///", "label": "Missing values",})

geo_price.explore("SwimmingPool",scheme="fisherjenks",legend=True,legend_kws={"caption": "Mean number of pools"})